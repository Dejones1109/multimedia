...
public class DCT
{
  ...
  {
    return ((u == 0) ? ((double) 1.0 / (double) Math.sqrt((double) 2.0))
                     : (double) 1.0);
  }
            ...
            for (double v = 0; v < 8; v++)
              {
                ...
                for (double u = 0; u < 8; u++)
                  innerloop += (DCT.C((int) u) / (double) 2.0)
                           * matrix[(int) v][(int) u]
                           * Math.cos((2 * x + 1) * u * Math.PI / (double) 16.0)
                           * Math.cos((2 * y + 1) * v * Math.PI / (double) 16.0);
    ...
    for (int u = 0; u < 8; u++)
      {
        for (int v = 0; v < 8; v++)
          {
            buffer[u][v] =  (float)(1.0 /4.0) * (float) C( u) * (float) C(v);
    ...
    for (int i = 1; i < 8; i++)
      ...
      for (int j = 0; j < 8; j++)
        {;
          ...
          c[i][j] =
            Math.sqrt(2.0 / 8.0)
            * Math.cos(((2.0 * jj + 1.0) * ii * Math.PI) / (16.0));
          cT[j][i] = c[i][j];
        }
      }
  }
